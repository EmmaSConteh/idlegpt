...
from idlelib import idleaiconfig
from idlelib import commenttranscript
from idlelib import commentdictation
...
class EditorWindow:
    ...
    def __init__(self, flist=None, filename=None, key=None, root=None):
        ...
        text.bind("<<open-ai-config>>", self.open_ai_config_event)
        text.bind("<<comment-dictation>>", self.comment_dictation_event)
        ...
        text.bind("<<open-chatgpt>>", self.open_chatgpt)
        self.idleai = self.AICompletion()
        
        self.ai_expand = self.AIExpand(self.text, self.idleai, self.text['font'])
        text.bind("<<code-suggest>>", self.ai_expand.expand_word_event)
    ...
    def open_ai_config_event(self, event):
        idleaiconfig.IdleAIConfig(self.top, self.text['font'])
        return "break"
    
    def comment_dictation_event(self, event):
        # Temp file to write audio date
        fd, name = tempfile.mkstemp(suffix=".wav")

        # Get the actual audio using the CommentDictation class
        with open(name, 'rb') as f:
            audio = f.read()
            buffer = io.BytesIO(audio)
            buffer.name = name
            commentdictation.CommentDictation(self.top, buffer, self.text['font'])

        # now that it is recorded, we can transcribe it
        commenttranscript.transcribe_comment(self, buffer, self.text['font'])
        os.close(fd)
        os.remove(name)
        return "break"

    def open_chatgpt(self, event):
        chatgpt.ChatGPT(self.top, self.text['font'])
        return "break"
